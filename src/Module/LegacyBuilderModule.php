...
final class LegacyBuilderModule implements Module {
	...
	public function build( ContainerBuilder $builder, BootstrapContext $context ): void {
		...
		$class_name = $config['plugin_class_name'] ?? $context->config()->get( 'plugin_class_name' );
		if ( ! is_string( $class_name ) || $class_name === '' ) {
			throw new \LogicException( 'LegacyBuilderModule requires "plugin_class_name" in module config or root config.' );
		}
		...
		$plugin_info->set_product_id( $context->config()->get( 'product_id', '' ) );
		$plugin_info->set_plugin_shops( $context->config()->get( 'shops', [] ) );
