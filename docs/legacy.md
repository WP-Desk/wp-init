...
Install `wpdesk/wp-builder` and configure `LegacyBuilderModule`:
...
`plugin_class_name` is required in the module config. There is no root-level legacy mode switch.
...
That is the main behavioral change: hookables become container-managed.
...
5. Move requirements, tracker, and licensing concerns into explicit modules.
